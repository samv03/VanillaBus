...
- After the first byte of a frame, a truncated header or payload times out
  in **2 s** (`invalid_length`) instead of hanging the session. Idle clients
  waiting for the next request are not timed out.
...
| `rx.batch` | engine event | `{ "frames": [FrameEvent, …], "dropped": <int> }` — after `bus.open`, ≤16–33 ms or ≤500 frames (whichever first). `dropped` is the cumulative engine RX drop-oldest count (queue cap 4096/bus). `rate_ms` is last inter-arrival ms, or `null` on the first sample per `(busId, can_id, is_eff)`. Known IDs may include `decode: { name, signals }` when a DBC is bound. |
